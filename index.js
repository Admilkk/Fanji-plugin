...
function createUpdateFile() {
    const filePath = './plugins/Fanji-plugin/apps/update.js';
    const fileContent = `import plugin from '../../../lib/plugins/plugin.js'
...
let log2 = await common.makeForwardMsg(this.e, [log, end], `${plugin}更新日志，共${line}条`)
    ...
    try {
        ...
        if (!fs.existsSync(filePath)) {
            ...
            console.log(`File '${filePath}' 创建成功.`);
        } else {
            console.log(`File '${filePath}' 已经存在.`);
        }
    } catch (error) {
        console.error(`更新更新失败: ${error.message}`);
    }
}
